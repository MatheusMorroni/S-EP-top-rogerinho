...
public class Particle {
    ...
    private int index;        // se é embulo index >= 0


    // create a new particle with given parameters        
    public Particle(double rx, double ry, double vx, double vy, double radius, double mass, Color color, int index) {
        this.vx = vx;
        this.vy = vy;
        this.rx = rx;
        this.ry = ry;
	...
	StdDraw.setPenColor(color);
        StdDraw.filledRectangle (rx, ry, radius, 0.5);
}
...
/*    public double timeToHitEmbulo() {
        if (index > 0){
            if          (vx < 0 && vx > vE)                 return (rE - rx - radius) / (vx - vE);
            else if     (vx > 0 && (vx > vE || vE < 0))     return (rE -rx - radius) / (vx - vE);
            else                                            return INFINITY;
        }
        else {
            if          (vx > 0 && vx < vE)                 return (rx - rE - radius) / (vE - vx);
            else if     (vx < 0 && (vx < vE || vE > 0))     return (rx -rE - radius) / (vE - vr);
            else                                            return INFINITY;
        }
    } */
    ...
    public void bounceOff(Particle that) {
        if (that.index() == 0 || this.index() == 0){
            double dx  = that.rx - this.rx;
            double dvx = that.vx - this.vx;
            double dvdr = dx*dvx;            // dv dot dr
...
/*    public void bounceOffEmbulo() {
        double dx  = rE - this.rx;
        double dvx = vE - this.vx;
        double dvdr = dx*dvx;       // dv dot dr
        double dist = this.radius            // distance between particle centers at collison

        // normal force F, and in x and y directions
        double F = 2 * this.mass * massE * dvdr / ((this.mass + massE) * dist);
        double fx = F * dx / dist;
